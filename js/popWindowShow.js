function showData() {
    ...
    //alert(document.getElementById("template_topic").value);
    ...
    for_data_show += '</table>';
    for_data_show += '<table width="100%">';
    for_data_show += '<tr><td>Name</td><td><input id="name" type="text" size="50x;"required/></td></tr>';
    for_data_show += ' <tr><td>Initiative</td><td><input id="initiative" type="text" size="50x;" required/></td></tr>';
    ...
    for_data_show += '<tr><td>Additional Properties</td><td><input id="additionalproperties" type="text" size="50x;" required/></td></tr>';
    ...
    for_data_show += '<br/><i>**Query pattern should be related to running template. SELECT clause must contain only variable shown in top right corner. Please, use full IRIs in query pattern.</i>';
    for_data_show += '<br/><i>***Additional properties represent properties of interest for running template. They must be entered as property1,property2,property3,...</i></td></tr>';
    for_data_show += '</table>';
    for_data_show += '<p><input type="button" value="Add" id="' + document.getElementById('get_templates').value + '" name="' + document.getElementById("template_topic").value + '" onclick="AddNewDataset(this.name,this.id);"align="right"/></p>';
    for_data_show += '</div>';
